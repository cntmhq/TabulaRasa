...
export function ProfileModal({ isOpen, onClose, profile, onUpdatePreferences }: ProfileModalProps) {
  ...
  // Keep the modal mounted briefly after isOpen flips false so the exit transition can play.
  useEffect(() => {
    if (isOpen) {
      setMounted(true);
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center px-4">
      <div
        onClick={onClose}
        className={`absolute inset-0 bg-black/80 backdrop-blur-sm transition-opacity duration-200 ${visible ? 'opacity-100 ease-out' : 'opacity-0 ease-in'}`}
      />
      <div
        className={`relative w-full max-w-md bg-[var(--color-brand-surface)] border border-[var(--color-brand-primary)] shadow-[0_0_30px_rgba(22,137,115,0.15)] rounded-xl overflow-hidden text-[var(--color-brand-glow)] transition-[opacity,transform] duration-200 will-change-[opacity,transform] ${visible ? 'opacity-100 scale-100 translate-y-0 ease-out' : 'opacity-0 scale-95 translate-y-3 ease-in'}`}
      >
